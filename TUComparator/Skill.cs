...
namespace TUComparatorLibrary
{
    internal class Skill
    {
        ...
        {
            string skillName = Comparator.skillData.Where(x => x.Element("id")?.Value.Equals(this.id) ?? false).FirstOrDefault()?.Element("name")?.Value ?? "";
            string allString = all ? " All" : "";
            string faction = Comparator.factionData.Where(x => x.Element("id")?.Value.Equals(this.faction.ToString()) ?? false).FirstOrDefault()?.Element("name")?.Value ?? "";

            string subSkillName = "";
            if (!String.IsNullOrEmpty(this.subSkill))
            {
                subSkillName = Comparator.skillData.Where(x => x.Element("id")?.Value.Equals(this.subSkill) ?? false).FirstOrDefault()?.Element("name")?.Value ?? "";
            }

            string subSkill2Name = "";
            if (!String.IsNullOrEmpty(this.subSkill2))
            {
                subSkill2Name = Comparator.skillData.Where(x => x.Element("id")?.Value.Equals(this.subSkill2) ?? false).FirstOrDefault()?.Element("name")?.Value ?? "";
            }
