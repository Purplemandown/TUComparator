        ...
        public string Compare()
        {
            ...
            switch (this.GetType())
            {
                ...
                default:
                    ...
                    {
                        // get faction string
                        string oldFaction = Comparator.factionData.Where(x => x.Element("id")?.Value.Equals(this.oldCard.faction) ?? false).FirstOrDefault()?.Element("name")?.Value ?? "";
                        string newFaction = Comparator.factionData.Where(x => x.Element("id")?.Value.Equals(this.newCard.faction) ?? false).FirstOrDefault()?.Element("name")?.Value ?? "";
